  ...
  THEN
	...
	THEN
		...
		SELECT
			...
			inf.var03                 , 
			inf.var04                 , 
			SUBSTR('prcts_regra_30i:' || inf.infst_outras_icms || '|' || inf.infst_val_cont || '|' || inf.infst_val_serv || '|' || inf.infst_base_icms || '|' || inf.infst_val_icms || '|' || inf.estb_cod || '|' || inf.infst_aliq_icms || '|' || inf.infst_tribicms || '|' || inf.infst_val_red || '>>'|| inf.var05,1,150) AS var05   , 
		...
		p_inf.update_reg        			  := 1;
	    p_inf.VAR05                           := SUBSTR('prcts_regra_30u:' || p_inf.estb_cod || '|' || p_inf.INFST_ISENTA_ICMS || '|' || p_inf.infst_val_cont || '|' || p_inf.INFST_VAL_SERV || '|' || p_inf.infst_val_red || '>>'|| p_inf.VAR05,1,150);
