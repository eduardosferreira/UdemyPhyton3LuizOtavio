...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_19_VAL_CONT_VAL_SERV'
...
PROMPT
PROMPT MAP_2_REGRA_19_VAL_CONT_VAL_SERV
	...
	WHERE   ${FILTRO}
		...
		AND (UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) NOT IN ('ASS') OR nf.mnfst_dtemiss >= TO_DATE('01/04/2017','DD/MM/YYYY'))
		  ...
		  AND EXISTS (SELECT 1 FROM  openrisow.item_nftl_serv    PARTITION (${PARTICAO_INF}) inf1 
					  ...
					  AND   NVL(inf1.INFST_VAL_CONT,0)              = NVL(inf1.INFST_BASE_ICMS,0) 
					    AND NVL(inf1.INFST_VAL_CONT,0)              < NVL(inf1.INFST_VAL_SERV,0) 
...
BEGIN
   ...
   LOOP
	...
	IF NVL(v_item_nftl_serv.INFST_VAL_CONT,0)              = NVL(v_item_nftl_serv.INFST_BASE_ICMS,0) 
	   AND NVL(v_item_nftl_serv.INFST_VAL_CONT,0)          < NVL(v_item_nftl_serv.INFST_VAL_SERV,0) 
	   AND NVL(v_item_nftl_serv.INFST_ISENTA_ICMS,0)       = 0 
	   AND NVL(v_item_nftl_serv.INFST_OUTRAS_ICMS,0)       = 0 
	   AND NVL(v_item_nftl_serv.INFST_VAL_DESC,0)          = 0 
	   AND NVL(v_item_nftl_serv.INFST_BASE_ICMS,0)         <> 0 
	THEN
		v_item_nftl_serv.update_reg        := 1;
		v_item_nftl_serv.VAR05             := SUBSTR('r2015_19i:' || v_item_nftl_serv.CFOP || '|' || v_item_nftl_serv.infst_val_cont || '|' || v_item_nftl_serv.INFST_VAL_SERV || '|' || v_item_nftl_serv.INFST_BASE_ICMS || '|' || v_item_nftl_serv.INFST_VAL_ICMS || '|' || v_item_nftl_serv.ESTB_COD || '|' || v_item_nftl_serv.INFST_ALIQ_ICMS || '|' || v_item_nftl_serv.INFST_TRIBICMS || '>>'|| v_item_nftl_serv.VAR05,1,150);
	...
	IF v_item_nftl_serv.update_reg     = 1 THEN
        ...
        v_inf.update_reg           			  := 1;
	    v_inf.VAR05                           := SUBSTR('r2015_19u:' || v_inf.INFST_OUTRAS_ICMS || '|' || v_inf.infst_val_cont || '|' || v_inf.INFST_VAL_SERV || '>>'|| v_inf.VAR05,1,150);
