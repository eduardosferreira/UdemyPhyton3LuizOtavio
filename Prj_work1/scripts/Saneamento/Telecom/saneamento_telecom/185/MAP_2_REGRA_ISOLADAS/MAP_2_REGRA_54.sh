...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_54'
...
PROMPT
PROMPT MAP_2_REGRA_54
PROMPT ### Inicio do processo ${0}  ###
PROMPT
DECLARE

   v_action_name VARCHAR2(32) := substr('MAP_2_REGRA_54',1,32);
   ...
   CURSOR c_sanea
	...
	WHERE   ${FILTRO}
		  ...
		  AND EXISTS (SELECT /*+ first_rows(1) */1 FROM  openrisow.item_nftl_serv    PARTITION (${PARTICAO_INF}) inf1 
					  ...
					  AND   NVL(inf1.INFST_BASE_ICMS,0)        >= (NVL(inf.INFST_BASE_ICMS,0) *(-1))
					  AND   NVL(inf1.INFST_VAL_ICMS,0)         >= (NVL(inf.INFST_VAL_ICMS,0) *(-1))
	...
	LOOP
		...
		IF v_bk_sanea.COUNT > 0 THEN
			...
			FOR i IN v_bk_sanea.FIRST .. v_bk_sanea.LAST LOOP 
				...
				SET   inf1.var05           = SUBSTR('R2015_54[' || v_bk_sanea(i).rowid_inf || ']:' || inf1.infst_val_cont || '|' || inf1.infst_val_serv || '|' || inf1.infst_base_icms || '|' || inf1.infst_val_icms || '>>'|| inf1.var05,1,150)
					
					, inf1.INFST_VAL_CONT  = NVL(inf1.INFST_VAL_CONT,0)  + v_bk_sanea(i).INFST_VAL_CONT
					, inf1.INFST_VAL_SERV  = NVL(inf1.INFST_VAL_SERV,0)  + v_bk_sanea(i).INFST_VAL_SERV
					, inf1.INFST_BASE_ICMS = NVL(inf1.INFST_BASE_ICMS,0) + v_bk_sanea(i).INFST_BASE_ICMS 
					, inf1.INFST_VAL_ICMS  = NVL(inf1.INFST_VAL_ICMS,0)  + v_bk_sanea(i).INFST_VAL_ICMS
				...
				AND   inf1.infst_dtemiss         = v_bk_sanea(i).mnfst_dtemiss
					  ...
					  AND   NVL(inf1.INFST_BASE_ICMS,0)        >= (NVL(v_bk_sanea(i).INFST_BASE_ICMS,0) *(-1))
					  AND   NVL(inf1.INFST_VAL_ICMS,0)         >= (NVL(v_bk_sanea(i).INFST_VAL_ICMS,0) *(-1))
				...
				IF SQL%FOUND THEN
					...
					SET inf.var05             = SUBSTR('R2015_54u:' || inf.infst_val_icms || '|' || inf.infst_base_icms || '|' || inf.infst_val_cont || '|' || inf.infst_val_serv || '|' || inf.infst_aliq_icms || '>>'|| inf.var05,1,150)
					  , inf.infst_val_icms    = 0
					  , inf.infst_base_icms   = 0
					  , inf.infst_val_cont    = 0 
					  , inf.infst_val_serv    = 0
					  , inf.infst_aliq_icms   = 0					  
