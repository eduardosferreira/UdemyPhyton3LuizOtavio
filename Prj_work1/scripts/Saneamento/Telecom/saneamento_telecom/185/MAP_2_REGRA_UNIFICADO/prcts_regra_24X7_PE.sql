...
-- MAP_2_24X7	
   ...
   v_cadg_tip_cli   openrisow.complvu_clifornec.cadg_tip_cli%type 	:= p_cli.cadg_tip_cli;
...
END IF;
	...
	IF p_nr_qtde_inf <= 1 THEN
		...
		IF v_cpf_cnpj != p_nf.cnpj_cpf THEN
		   v_nr_add_nf             := 1;	
		   p_nf.update_reg         := 1;	
		   p_nf.var05              := SUBSTR('24X7_PE>>ANT_CPF_CNPJ=' || p_nf.cnpj_cpf  || '>>' ||p_nf.var05,1,150);
		...
		IF v_cpf_cnpj != p_cli.cadg_cod_cgccpf THEN
			v_nr_add_cli           := 1;
			p_cli.update_reg       := 1;
			p_cli.var05_cli        := SUBSTR('24X7_PE>>CLI: ' || p_cli.cadg_cod_cgccpf || '|' || p_cli.cadg_tip  || '>>' ||p_cli.var05_cli,1,150);
	...
	THEN
		...
		IF NVL(p_cli.cadg_tip_cli,'X') NOT IN ('01','02','03','04','05','06','07','08','99')
		THEN 
		...
		END IF;
                ...
                IF NVL(TRIM(p_cli.cadg_tip_assin),'X') NOT IN ('1','2','3','4','5','6')
		THEN
	...
	IF v_cadg_tip_cli != NVL(p_cli.cadg_tip_cli,'X') OR v_cadg_tip_assin != NVL(p_cli.cadg_tip_assin,'X')  THEN
		...
		p_cli.var05_comp           := SUBSTR('24X7_PE: ' || p_cli.cadg_tip_assin || '|' || p_cli.cadg_tip_cli  || '>>' ||p_cli.var05_cli,1,150);
	...
	IF p_nf.cnpj_cpf != p_inf.cgc_cpf OR p_cli.cadg_cod_insest != p_inf.ie OR v_num_terminal != p_inf.infst_tel THEN
	   v_nr_add_inf             := 1;
	   p_inf.update_reg         := 1;	
	   p_inf.var05              := SUBSTR('24X7_PE>>ANT_CGC_CPF=' ||'_IE='||p_inf.IE||'_INFST_TEL='||p_inf.INFST_TEL|| '>>' ||p_inf.var05,1,150);
	...
	/*IF p_cli.CADG_DAT_ATUA >= TO_DATE('01/01/2017','DD/MM/YYYY') AND NVL(p_cli.cadg_tip_assin,0)  != 0 THEN
		p_cli.update_reg_comp     := 1;
		p_cli.var05_comp           := SUBSTR('24X7_PE:TIP_ASS[0]: ' || p_cli.cadg_tip_assin  || '>>' ||p_cli.var05_cli,1,150);
