...
if ambiente == 'PROD' :
    ...
    owner_gfcarga = ''
    owner_openrisow = ''
    dir_entrada_carga = os.path.join( dir_base, 'entrada', os.path.realpath('.').split('/')[-1] )
    dir_arquivos_protocolados  = os.path.join( dir_base, 'entrada', os.path.realpath('.').split('/')[-1],'ARQUIVOS_PROTOCOLADO' )
    userBD = ''
    pwdBD = ''
