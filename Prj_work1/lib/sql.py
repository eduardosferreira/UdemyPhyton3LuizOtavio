...
class SQL() :
    ...
    def troca_owner(self, query):
        query = query.replace('gfcadastro.', self.__configuracoes.owner_gfcadastro.strip() + ( '.' if not self.__configuracoes.owner_gfcadastro.strip().endswith('.') else '' ) )
        query = query.replace('GFCADASTRO.', self.__configuracoes.owner_gfcadastro.strip() + ( '.' if not self.__configuracoes.owner_gfcadastro.strip().endswith('.') else '' ) )

        query = query.replace('gfcarga.', self.__configuracoes.owner_gfcarga.strip() + ( '.' if not self.__configuracoes.owner_gfcarga.strip().endswith('.') else '') )
        query = query.replace('GFCARGA.', self.__configuracoes.owner_gfcarga.strip() + ( '.' if not self.__configuracoes.owner_gfcarga.strip().endswith('.') else '') )
        
        query = query.replace('openrisow.', self.__configuracoes.owner_openrisow.strip() + ( '.' if not self.__configuracoes.owner_openrisow.strip().endswith('.') else '') )
        query = query.replace('OPENRISOW.', self.__configuracoes.owner_openrisow.strip() + ( '.' if not self.__configuracoes.owner_openrisow.strip().endswith('.') else '') )
