...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_29_REALOCACAO_OUTRAS_SEM_FILTRO_ISENTAS'
...
PROMPT
PROMPT MAP_2_REGRA_29_REALOCACAO_OUTRAS_SEM_FILTRO_ISENTAS
...
BEGIN
   ...
   LOOP
	...
	THEN		
		...
		v_item_nftl_serv.INFST_ISENTA_ICMS := 0;
	...
	IF v_item_nftl_serv.update_reg     = 1 THEN
        ...
        v_inf.update_reg           			  := 1;
	    v_inf.VAR05                           := SUBSTR('r2015_29u:' || v_inf.INFST_OUTRAS_ICMS || '|' || v_inf.infst_val_cont || '|' || v_inf.INFST_VAL_SERV || '>>'|| v_inf.VAR05,1,150);
        v_inf.INFST_OUTRAS_ICMS    			  := 0;
        v_inf.INFST_VAL_SERV      			  := NVL(v_inf.INFST_BASE_ICMS,0) + NVL(v_inf.INFST_ISENTA_ICMS,0);
        v_inf.INFST_VAL_CONT      			  := NVL(v_inf.INFST_BASE_ICMS,0) + NVL(v_inf.INFST_ISENTA_ICMS,0);		
    END IF;	
