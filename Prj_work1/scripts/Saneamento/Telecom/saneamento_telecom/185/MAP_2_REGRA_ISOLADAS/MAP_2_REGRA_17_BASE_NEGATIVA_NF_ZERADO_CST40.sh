...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_17_BASE_NEGATIVA_NF_ZERADO_CST40'
...
PROMPT
PROMPT MAP_2_REGRA_17_BASE_NEGATIVA_NF_ZERADO_CST40
...
DECLARE   
	...
	WHERE   ${FILTRO}
		...
		AND (UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) NOT IN ('ASS') OR nf.mnfst_dtemiss >= TO_DATE('01/04/2017','DD/MM/YYYY'))
		  ...
		  AND NVL(nf.MNFST_VAL_TOT,0)     = 0 
		  AND NVL(nf.MNFST_VAL_BASICMS,0) = 0 
...
BEGIN
   ...
   LOOP
	...
	UPDATE openrisow.item_nftl_serv inf
		SET inf.VAR05                                           = SUBSTR('r2015_17:' || inf.ESTB_COD || '>>'|| inf.VAR05,1,150)
          , inf.ESTB_COD                                        = '40'
