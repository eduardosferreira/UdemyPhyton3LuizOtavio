...
BEGIN
	...
	IF (     
			...
			NVL(p_inf.INFST_OUTRAS_ICMS,0) <> 0
	...
	THEN
		...
		v_inf_insert.var05             := SUBSTR('prcts_regra_48i:' || v_inf_insert.cfop || '|' || v_inf_insert.infst_val_cont || '|' || v_inf_insert.infst_val_serv || '|' || v_inf_insert.infst_base_icms || '|' || v_inf_insert.infst_val_icms || '|' || v_inf_insert.estb_cod || '|' || v_inf_insert.infst_aliq_icms || '|' || v_inf_insert.infst_tribicms || '>>'|| v_inf_insert.var05,1,150);
		v_inf_insert.INFST_VAL_CONT    := v_inf_insert.INFST_OUTRAS_ICMS;
		v_inf_insert.INFST_VAL_SERV    := v_inf_insert.INFST_OUTRAS_ICMS;
		...
		p_inf.update_reg        			  := 1;
		p_inf.var05                           := SUBSTR('prcts_regra_48u:' || p_inf.infst_outras_icms || '|' || p_inf.infst_val_cont || '|' || p_inf.infst_val_serv || '>>'|| p_inf.var05,1,150);
