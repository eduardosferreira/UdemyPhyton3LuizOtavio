...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_INSERIR_TMP_CLI_DBLINK_TMP_CADASTRO_CLI_COMP.sh'
...
PROMPT
PROMPT MAP_2_REGRA_INSERIR_TMP_CLI_DBLINK_TMP_CADASTRO_CLI_COMP.sh
PROMPT ### Inicio do processo ${0} - ${SERIE}  ###
PROMPT

DECLARE
   v_action_name VARCHAR2(32) := substr('MAP_2_REGRA_INSERIR_TMP_CLI_DBLINK_TMP_CADASTRO_CLI_COMP.sh',1,32);
   ...
   CURSOR c_sanea
       IS
	WITH TMP_TAB1 AS (	SELECT DISTINCT A.ROWID_CLI
	...
	SELECT /*+ PARALLEL (15) */ A.* FROM TMP_TAB1 T, OPENRISOW.CLI_FORNEC_TRANSP@C7 A 
	WHERE A.ROWID = T.ROWID_CLI
	;
	   ...
	   LOOP
			...
			IF v_bk_sanea.COUNT > 0 THEN
				FORALL i IN v_bk_sanea.FIRST .. v_bk_sanea.LAST
					INSERT   INTO  OPENRISOW.TMP_K_C7_CLI_FORNEC_TRANSP 
					VALUES v_bk_sanea(i);  
