...
#### Monta caminho e nome do destino
    ...
    ret = 0
    dir_arquivos = disco + configuracoes.diretorio_arquivos + ufi + SD
...
#        print arquivo.replace("á","a").replace("é","e").replace("í","i").replace("ó","o").replace("ú","u").replace("ã","a").replace("ç","c").replace(" ","_").replace(",","").replace("õ","o");
    ...
    log('Diretorio.:',dir_arquivos)
    listaregerados = lista_arquivos(mascara_regerado,dir_arquivos)
 ...
 #   log("###DEBUG###, listaprotocolados = ",listaprotocolados )
    ...
    for arquivo in listaregerados:
        ...
        if (iea and am_d and amd_g and hms_g):
            ...
            nova_pasta = os.path.join(dir_regerados,aa,ma)
            ...
            bad_nome_novo  = os.path.join(dir_arquivos ,"GIA"+ ufi + "_" + nomd + "_" + iea + "_REG_V" + proximo + ".BAD")
            ...
            if (os.path.isfile(bad_nome_velho)):
                shutil.move(bad_nome_velho,bad_nome_novo, copy_function = shutil.copytree)
    ...
    for arquivo in listaprotocolados:
        ...
        if (iea and am_d and amd_g and hms_g):
            ...
            ma = am_d[4:]
            nomd = am_d[4:]+ am_d[0:4]
            nova_pasta = os.path.join(dir_protocolados,aa,ma)
