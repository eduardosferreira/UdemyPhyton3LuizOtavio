...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_26_AJUSTE_VAL_SERV_ZERADO_VAL_CONT'
...
PROMPT
PROMPT MAP_2_REGRA_26_AJUSTE_VAL_SERV_ZERADO_VAL_CONT
...
DECLARE
   ...
   CURSOR c_inf
	...
	AND (UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) NOT IN ('ASS') OR nf.mnfst_dtemiss >= TO_DATE('01/04/2017','DD/MM/YYYY'))
		  ...
		  AND nvl(inf.INFST_VAL_CONT,0)      =   nvl(inf.INFST_BASE_ICMS,0)
...
BEGIN
   ...
   LOOP
	  ...
	  SET inf.var05             = substr('r2015_26u' ||':' || inf.INFST_VAL_SERV ||'>>'||inf.var05,1,150) 			
