...
BEGIN
  IF ( p_sanea.serie IN ('UK', 'UT') ) 
  THEN
  
    IF (NVL(p_inf.INFST_BASE_ICMS,0)             <> 0  OR  NVL(p_inf.INFST_VAL_ICMS,0)              <> 0)
	...
	THEN
		...
		SELECT
			...
			'40' AS estb_cod          , 
			inf.infst_dsc_compl       , 
			inf.INFST_ISENTA_ICMS  AS infst_val_cont        , 
			inf.INFST_ISENTA_ICMS  AS infst_val_serv        , 
			0 AS infst_val_desc       , 
			...
			inf.var03                 , 
			inf.var04                 , 
			SUBSTR('prcts_regra_43i:' || inf.infst_val_red  || '|' || inf.infst_outras_icms || '|' || inf.infst_val_cont || '|' || inf.infst_val_serv || '|' || inf.infst_base_icms || '|' || inf.infst_val_icms || '|' || inf.estb_cod || '|' || inf.infst_aliq_icms || '|' || inf.infst_tribicms || '>>'|| inf.var05,1,150) AS var05   , 
		...
		p_inf.update_reg        			  := 1;
	        p_inf.VAR05                           := SUBSTR('prcts_regra_43u:' || p_inf.INFST_ISENTA_ICMS || '|' || p_inf.infst_val_cont || '|' || p_inf.INFST_VAL_SERV|| '|' || p_inf.ESTB_COD|| '|' || p_inf.INFST_TRIBICMS || '|' || p_inf.INFST_VAL_RED || '>>'|| p_inf.VAR05,1,150);
        

		p_inf.INFST_ISENTA_ICMS               := 0;
		p_inf.INFST_VAL_CONT                  := p_inf.INFST_BASE_ICMS;
		p_inf.INFST_VAL_SERV                  := NVL(p_inf.INFST_BASE_ICMS,0) +  NVL(p_inf.INFST_VAL_DESC,0); -- (Item original)

		p_inf.ESTB_COD 			      := '00';
		p_inf.INFST_TRIBICMS 		      := 'S';
	
		p_inf.INFST_VAL_RED                   := NULL;
