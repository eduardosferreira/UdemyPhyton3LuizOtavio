...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_16_BASE_NEGATIVA'
...
PROMPT
PROMPT MAP_2_REGRA_16_BASE_NEGATIVA
	...
	WHERE   ${FILTRO}
		...
		AND (UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) NOT IN ('ASS') OR nf.mnfst_dtemiss >= TO_DATE('01/04/2017','DD/MM/YYYY'))
		  ...
		  AND ( NVL(inf.infst_outras_icms,0) = 0 
			  ...
			  ) -- Nro da Regra 16) Base Negativa menor que 0,1 centavos
          AND ( 
		        ( UPPER(TRANSLATE(nf.mnfst_serie,'x ','x'))           = 'UT' 
		          AND NVL(inf.infst_base_icms,0)                      > -0.35 
				  AND NVL(inf.infst_base_icms,0) < 0 
				) 
			    OR ( UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) = '1' AND NVL(inf.infst_base_icms,0) = -0.01 ) 
			  );
...
BEGIN
   ...
   LOOP
	...
	UPDATE openrisow.item_nftl_serv inf
		SET inf.VAR05                     = SUBSTR('r2015_16:' || inf.infst_aliq_icms || '|' || inf.infst_tribicms || '>>'|| inf.VAR05,1,150)
          , inf.infst_aliq_icms           = 25
