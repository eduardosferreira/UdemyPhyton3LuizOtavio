...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_42'
...
PROMPT
PROMPT MAP_2_REGRA_42
PROMPT ### Inicio do processo ${0} - ${SERIE}  ###
PROMPT

DECLARE   
    
	CURSOR c_par(p_emps_cod IN openrisow.item_nftl_serv.emps_cod%type, 
				 ...
				 p_SERV_COD IN openrisow.item_nftl_serv.SERV_COD%type,
				 p_infst_num_seq IN openrisow.item_nftl_serv.infst_num_seq%type
				 ) 
	   IS
		SELECT 	inf1.rowid             rowid_inf, 
			    to_NUMBER(0) 		   update_reg,
				inf1.infst_num_seq AS   infst_num_seq_max,
				inf1.* 
		              FROM  openrisow.item_nftl_serv    PARTITION (${PARTICAO_INF}) inf1 
					  WHERE inf1.emps_cod                                                   = p_emps_cod
						  AND   inf1.fili_cod                                               = p_fili_cod
						  AND   inf1.infst_serie                                            = p_infst_serie
						  AND   inf1.infst_num                                              = p_infst_num
						  AND   inf1.infst_dtemiss                                          = p_infst_dtemiss
						  AND   inf1.SERV_COD             									= p_SERV_COD 
						  AND   NOT EXISTS (SELECT 1 FROM GFCADASTRO.tmp_rowid R WHERE R.ID_ROWID = inf1.rowid)
						  AND   inf1.ESTB_COD             = '90'
						  AND   inf1.INFST_TRIBICMS       = 'P'
						  AND   REGEXP_LIKE(inf1.CFOP, '^53|^63|^73') 
						  AND   NVL(inf1.INFST_VAL_CONT,0)       < 0
						  AND   NVL(inf1.INFST_VAL_SERV,0)       < 0
						  AND   NVL(inf1.INFST_OUTRAS_ICMS,0)    < 0
						  AND   NVL(inf1.INFST_VAL_DESC,0)       = 0
						  AND   NVL(inf1.INFST_ALIQ_ICMS,0)      = 0
						  AND   NVL(inf1.INFST_BASE_ICMS,0)      = 0
						  AND   NVL(inf1.INFST_VAL_ICMS,0)       = 0
						  AND   NVL(inf1.INFST_ISENTA_ICMS,0)    = 0
						  AND EXISTS (SELECT /*+ first_rows(1)*/1	 FROM openrisow.ITEM_NFTL_SERV inf2
										WHERE inf2.EMPS_COD            = inf1.EMPS_COD
										  AND inf2.FILI_COD            = inf1.FILI_COD
										  AND inf2.INFST_DTEMISS       = inf1.INFST_DTEMISS
										  AND inf2.INFST_SERIE         = inf1.INFST_SERIE
										  AND inf2.INFST_NUM           = inf1.INFST_NUM
										  AND inf2.SERV_COD            = inf1.SERV_COD
										  AND inf2.infst_num_seq       = p_infst_num_seq
										  AND inf2.ESTB_COD            = '20'
										  AND ABS(NVL(inf2.INFST_VAL_CONT,0)) <> ABS(NVL(inf1.INFST_VAL_CONT,0))
										  AND ABS(NVL(inf2.INFST_VAL_SERV,0)) <> ABS(NVL(inf1.INFST_VAL_SERV,0))
										  AND NVL(inf2.INFST_VAL_CONT,0)      > 0
										  AND NVL(inf2.INFST_VAL_SERV,0)      > 0
										  AND NVL(inf2.INFST_VAL_DESC,0)      = 0
										  AND NVL(inf2.INFST_ALIQ_ICMS,0)     <> 0
										  AND NVL(inf2.INFST_BASE_ICMS,0)     > 0
										  AND NVL(inf2.INFST_VAL_ICMS,0)      > 0
										  AND (NVL(inf2.INFST_ISENTA_ICMS,0)  > 0 AND NVL(inf2.INFST_ISENTA_ICMS,0) >= ABS(NVL(inf1.INFST_OUTRAS_ICMS,0)))
										  AND NVL(inf2.INFST_OUTRAS_ICMS,0)   = 0
									);
											  
	v_par                 c_par%ROWTYPE;
	v_item_nftl_serv      c_par%ROWTYPE;	
	...
	WHERE   ${FILTRO}
		...
		AND (UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) NOT IN ('ASS') OR nf.mnfst_dtemiss >= TO_DATE('01/04/2017','DD/MM/YYYY'))
		  AND nf.mnfst_dtemiss >= TO_DATE('01/01/2017','DD/MM/YYYY') 
		  AND nf.mnfst_dtemiss <= TO_DATE('31/12/2017','DD/MM/YYYY')	
...
BEGIN
   ...
   LOOP
	...
	v_item_nftl_serv.infst_num_seq_max    := v_inf.infst_num_seq_max;
	IF 	   v_inf.fl_update = 'S'
	THEN		
		OPEN c_par(p_emps_cod => v_inf.emps_cod, 
			     p_fili_cod => v_inf.fili_cod, 
				 p_infst_serie => v_inf.infst_serie,
				 p_infst_num => v_inf.infst_num,
				 p_infst_dtemiss => v_inf.infst_dtemiss,
				 p_SERV_COD => v_inf.SERV_COD,
				 p_infst_num_seq => v_inf.infst_num_seq
				 );
		FETCH c_par
		INTO  v_par;		
		IF c_par%FOUND THEN
			INSERT INTO GFCADASTRO.tmp_rowid (ID_ROWID) VALUES (v_par.ROWID_INF);
			v_item_nftl_serv                    := v_par;	
			v_item_nftl_serv.update_reg         := 1; 
			v_item_nftl_serv.VAR05              := SUBSTR('r2015_42u:' || v_item_nftl_serv.INFST_VAL_CONT || '|' || v_item_nftl_serv.INFST_VAL_SERV || '|' || v_item_nftl_serv.INFST_ISENTA_ICMS || '|' || v_item_nftl_serv.INFST_OUTRAS_ICMS || '|' || v_item_nftl_serv.VAR05,1,150);
			
			v_item_nftl_serv.INFST_VAL_CONT     := v_item_nftl_serv.INFST_VAL_CONT * (-1);
			v_item_nftl_serv.INFST_VAL_SERV     := v_item_nftl_serv.INFST_VAL_SERV * (-1);
			v_item_nftl_serv.INFST_ISENTA_ICMS  := v_item_nftl_serv.INFST_ISENTA_ICMS * (-1);
			v_item_nftl_serv.INFST_OUTRAS_ICMS  := v_item_nftl_serv.INFST_OUTRAS_ICMS * (-1);
	...
	IF v_item_nftl_serv.update_reg     = 1 THEN
        ...
        v_inf.update_reg           			  := 1;
	    v_inf.VAR05                           := SUBSTR('r2015_42i:' || v_inf.INFST_VAL_CONT || '|' || v_inf.INFST_VAL_SERV || '|' || v_inf.INFST_ISENTA_ICMS || '|' || v_inf.VAR05,1,150);
        v_inf.INFST_VAL_CONT      			  := v_inf.INFST_VAL_CONT - ABS(v_item_nftl_serv.INFST_OUTRAS_ICMS);	
        v_inf.INFST_VAL_SERV      			  := v_inf.INFST_VAL_SERV - ABS(v_item_nftl_serv.INFST_OUTRAS_ICMS);	
        v_inf.INFST_ISENTA_ICMS      		  := v_inf.INFST_ISENTA_ICMS - ABS(v_item_nftl_serv.INFST_OUTRAS_ICMS);
