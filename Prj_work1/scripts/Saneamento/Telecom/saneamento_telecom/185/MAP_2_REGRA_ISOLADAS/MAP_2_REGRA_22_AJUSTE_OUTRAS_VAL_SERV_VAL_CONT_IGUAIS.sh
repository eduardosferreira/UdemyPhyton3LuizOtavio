...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_22_AJUSTE_OUTRAS_VAL_SERV_VAL_CONT_IGUAIS'
...
PROMPT
PROMPT MAP_2_REGRA_22_AJUSTE_OUTRAS_VAL_SERV_VAL_CONT_IGUAIS.sh
...
DECLARE
   ...
   CURSOR c_inf
    ...
    WHERE ${FILTRO}
	      AND UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) IN ('05','7') 
		...
		AND (UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) NOT IN ('ASS') OR nf.mnfst_dtemiss >= TO_DATE('01/04/2017','DD/MM/YYYY'))
          ...
          AND nvl(inf.INFST_OUTRAS_ICMS,0) <>   nvl(inf.INFST_VAL_SERV,0)
...
BEGIN
   ...
   LOOP
	  ...
	  SET inf.var05             = substr('r2015_22u' ||':'|| inf.infst_outras_icms || '>>'||inf.var05,1,150) 			
