...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_20_ACERTO_SERIE_05_CRIA_VALOR_OUTRAS'
...
PROMPT
PROMPT MAP_2_REGRA_20_ACERTO_SERIE_05_CRIA_VALOR_OUTRAS
   ...
   CURSOR c_inf
    ...
    WHERE ${FILTRO}
		...
		AND (UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) NOT IN ('ASS') OR nf.mnfst_dtemiss >= TO_DATE('01/04/2017','DD/MM/YYYY'))
		  ...
		  AND nvl(inf.INFST_ALIQ_ICMS,0)    = 25 
		  AND inf.ESTB_COD                  = '90'
		  AND nvl(inf.CFOP,'_0_')           <>'0000'
          AND nvl(inf.INFST_BAS_PISCOF,0)   <> 0; 
    v_inf                 c_inf%ROWTYPE;
...
BEGIN
   ...
   LOOP
	  ...
	  v_inf.INFST_OUTRAS_ICMS := v_inf.INFST_BAS_PISCOF;
	  v_inf.INFST_VAL_SERV    := v_inf.INFST_BAS_PISCOF;
	  v_inf.INFST_VAL_CONT    := v_inf.INFST_BAS_PISCOF;
	  v_inf.INFST_ALIQ_ICMS   := 0;
	  -- v_inf.CFOP              := '0000';
	  v_inf.INFST_TRIBICMS    := 'P';

	  UPDATE openrisow.item_nftl_serv inf
	  SET inf.var05             = substr('r2015_20u' ||':'|| inf.infst_outras_icms || '|' || inf.INFST_ALIQ_ICMS || '|' || inf.infst_val_serv || '|' || inf.infst_val_cont || '|' || inf.CFOP|| '|' || inf.INFST_TRIBICMS ||'>>'||inf.var05,1,150) 			
