...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_43_ISENTA_NEGATIVO_REDUCAO_BASE'
...
PROMPT
PROMPT MAP_2_REGRA_43_ISENTA_NEGATIVO_REDUCAO_BASE
PROMPT ### Inicio do processo ${0} - ${SERIE}  ###
PROMPT

DECLARE   
 
	v_action_name VARCHAR2(32) := substr('MAP_2_REGRA_43_ISENTA_NEGATIVO_REDUCAO_BASE',1,32);
	...
	WHERE   ${FILTRO}
	      AND UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) IN ('UK', 'UT') 
		...
		AND (UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) NOT IN ('ASS') OR nf.mnfst_dtemiss >= TO_DATE('01/04/2017','DD/MM/YYYY'))
		  ...
		  AND EXISTS (SELECT 1 FROM  openrisow.item_nftl_serv    PARTITION (${PARTICAO_INF}) inf1 
					  ...
					  AND  (NVL(inf1.INFST_BASE_ICMS,0)             <> 0 
					    OR  NVL(inf1.INFST_VAL_ICMS,0)              <> 0)
...
BEGIN
   ...
   LOOP
	...
	IF (    NVL(v_inf.INFST_BASE_ICMS,0)             <> 0 
		OR  NVL(v_inf.INFST_VAL_ICMS,0)              <> 0)
	...
	THEN
		...
		v_item_nftl_serv.VAR05             := SUBSTR('r2017_43i:' || v_item_nftl_serv.INFST_VAL_DESC || '|' || v_item_nftl_serv.infst_val_cont || '|' || v_item_nftl_serv.INFST_VAL_SERV || '|' || v_item_nftl_serv.INFST_BASE_ICMS || '|' || v_item_nftl_serv.INFST_VAL_ICMS || '|' || v_item_nftl_serv.ESTB_COD || '|' || v_item_nftl_serv.INFST_ALIQ_ICMS || '|' || v_item_nftl_serv.INFST_TRIBICMS || '|' || v_item_nftl_serv.INFST_VAL_RED || '>>'|| v_item_nftl_serv.VAR05,1,150);
        
		v_item_nftl_serv.INFST_BASE_ICMS   := 0;
		v_item_nftl_serv.INFST_VAL_ICMS    := 0;
		v_item_nftl_serv.INFST_VAL_CONT    := v_item_nftl_serv.INFST_ISENTA_ICMS; -- (Item original)
		v_item_nftl_serv.INFST_VAL_SERV    := v_item_nftl_serv.INFST_ISENTA_ICMS; -- (Item original)
	    ...
	    v_item_nftl_serv.INFST_VAL_RED     := NULL;
        ...
        v_inf.update_reg           			  := 1;
	    v_inf.VAR05                           := SUBSTR('r2017_43u:' || v_inf.INFST_ISENTA_ICMS || '|' || v_inf.infst_val_cont || '|' || v_inf.INFST_VAL_SERV|| '|' || v_inf.ESTB_COD|| '|' || v_inf.INFST_TRIBICMS || '|' || v_inf.INFST_VAL_RED || '>>'|| v_inf.VAR05,1,150);
        
		v_inf.INFST_ISENTA_ICMS               := 0;
		v_inf.INFST_VAL_CONT                  := v_inf.INFST_BASE_ICMS;
		v_inf.INFST_VAL_SERV                  := NVL(v_inf.INFST_BASE_ICMS,0) -- (Item original) 
												 +  NVL(v_inf.INFST_VAL_DESC,0); -- (Item original)

		v_inf.ESTB_COD 						  := '00';
		v_inf.INFST_TRIBICMS 				  := 'S';
	
		v_inf.INFST_VAL_RED                   := NULL;
