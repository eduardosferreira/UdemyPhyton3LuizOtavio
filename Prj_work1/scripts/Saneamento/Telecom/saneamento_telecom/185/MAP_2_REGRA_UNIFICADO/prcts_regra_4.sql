-- CREATE OR REPLACE 
PROCEDURE prcts_regra_4(p_inf         IN OUT NOCOPY c_inf%rowtype)
AS
  -- MAP_2_REGRA_4_AJUSTE_ITEM_0_TOT_PREEN
BEGIN
  ...
  THEN
	IF nvl(p_inf.infst_val_cont,0)     = nvl(p_inf.infst_outras_icms,0)   
	...
	THEN
      p_inf.update_reg        := 1;
      p_inf.var05             := SUBSTR('prcts_regra_4:' || p_inf.infst_outras_icms || '|' || p_inf.infst_val_cont || '>>' ||p_inf.var05,1,150);
	  p_inf.infst_val_cont    := nvl(p_inf.infst_val_serv,0);
	  p_inf.infst_outras_icms := nvl(p_inf.infst_val_serv,0);	
