...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_41_AJUSTE_VLR_REDUCAO_BASEADO_ISENTAS'
...
PROMPT
PROMPT MAP_2_REGRA_41_AJUSTE_VLR_REDUCAO_BASEADO_ISENTAS
...
DECLARE
   ...
   CURSOR c_sanea--(p_mnfst_dtemiss openrisow.mestre_nftl_serv.mnfst_dtemiss%TYPE)
	...
	AND (UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) NOT IN ('ASS') OR nf.mnfst_dtemiss >= TO_DATE('01/04/2017','DD/MM/YYYY'))
		...
		AND (NVL(inf.infst_base_icms,0) <> 0 or NVL(inf.infst_val_icms,0) <> 0)
		AND NVL(inf.infst_val_red,0) <> NVL(inf.infst_isenta_icms,0)
		AND NVL(inf.infst_val_cont,0) = NVL(inf.infst_base_icms,0) + NVL(inf.infst_isenta_icms,0) 
...
BEGIN
  ...
  THEN
	...
	THEN
        p_altera_inf             := TRUE;
		p_inf.var05              := substr('r2016_41u:' || p_inf.infst_val_red       || '|' 
														|| p_inf.infst_isenta_icms   || '>>' ||p_inf.VAR05,1,150) ;	
