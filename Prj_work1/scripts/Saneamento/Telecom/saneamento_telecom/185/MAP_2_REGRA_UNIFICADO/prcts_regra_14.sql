...
BEGIN
	...
	THEN
		...
		SELECT
			...
			'90' AS estb_cod          , 
			inf.infst_dsc_compl       , 
			inf.infst_outras_icms AS infst_val_cont        , 
			inf.infst_outras_icms AS infst_val_serv        , 
			inf.infst_val_desc       , 
			...
			inf.var03                 , 
			inf.var04                 , 
			SUBSTR('prcts_regra_14i:' || inf.cfop || '|' || inf.infst_val_cont || '|' || inf.infst_val_serv || '|' || inf.infst_base_icms || '|' || inf.infst_val_icms || '|' || inf.estb_cod || '|' || inf.infst_aliq_icms || '|' || inf.infst_tribicms || '>>'|| inf.var05,1,150) AS var05   , 
		...
		p_inf.update_reg        			  := 1;
	    p_inf.var05                           := SUBSTR('prcts_regra_14u:' || p_inf.infst_outras_icms || '|' || p_inf.infst_val_cont || '|' || p_inf.infst_val_serv || '>>'|| p_inf.var05,1,150);
