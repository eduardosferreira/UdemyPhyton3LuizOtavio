...
--Solução proposta:
--
--Criar um novo item com CST 90 (OUTRAS_ICMS), utilizando o valor de OUTRAS_ICMS existente no item totalmente tributado (CST 00) e após atualizar a linha de origem zerando o valor outras_icms e atualizar os campos de val_serv e val_cont com o valor de base_icms.
--
--Para: 
--	 AND it.ESTB_COD         = '00'
--	 AND it.INFST_VAL_CONT   <> 0
--	 AND it.INFST_VAL_SERV   = it.INFST_VAL_CONT 
--	 AND it.INFST_ALIQ_ICMS  <> 0
--	 AND it.INFST_BASE_ICMS  <> 0
--	 AND it.INFST_VAL_ICMS   <> 0
--        AND it.INFST_OUTRAS_ICMS <> 0
--	 AND it.INFST_VAL_DESC    = 0
--	 AND it.INFST_ISENTA_ICMS = 0
--	 AND it.INFST_VAL_SERV    = it.INFST_BASE_ICMS + it.INFST_OUTRAS_ICMS 	

BEGIN
  IF    p_inf.rowid_inf IS NOT NULL 
  THEN 
    IF 
			NVL(p_inf.ESTB_COD,'')         = '00'
		...
		AND NVL(p_inf.INFST_VAL_SERV,0)    = (NVL(p_inf.INFST_BASE_ICMS,0) + NVL(p_inf.INFST_OUTRAS_ICMS,0)) 
	THEN
		...
		--	Cria um novo item CST 90 (OUTRAS_ICMS), com valor de outras_icms da linha de origem;
		v_inf_insert.var05             := SUBSTR('prcts_regra_60i:' || v_inf_insert.ESTB_COD || '|' || v_inf_insert.INFST_OUTRAS_ICMS || '|' || v_inf_insert.infst_val_cont || '|' || v_inf_insert.infst_val_serv || '|' || v_inf_insert.INFST_TRIBICMS || '|' || v_inf_insert.INFST_VAL_PIS || '|' || v_inf_insert.INFST_VAL_COFINS  || '>>'|| v_inf_insert.var05,1,150);
		v_inf_insert.ESTB_COD          := '90';
		v_inf_insert.INFST_VAL_SERV    := NVL(v_inf_insert.INFST_OUTRAS_ICMS,0);
		v_inf_insert.INFST_VAL_CONT    := NVL(v_inf_insert.INFST_OUTRAS_ICMS,0);
		v_inf_insert.INFST_TRIBICMS    := 'P';
		v_inf_insert.INFST_VAL_PIS	   := 0;
		v_inf_insert.INFST_VAL_COFINS  := 0;
		v_inf_insert.INFST_ALIQ_ICMS   := 0;
		v_inf_insert.INFST_BASE_ICMS   := 0;
		v_inf_insert.INFST_VAL_ICMS    := 0;
        ...
        p_inf.update_reg         := 1;
        p_inf.var05              := SUBSTR('prcts_regra_60u:' || p_inf.INFST_VAL_SERV || '|' || p_inf.INFST_VAL_CONT || '|' || p_inf.INFST_OUTRAS_ICMS || '>>' || p_inf.var05,1,150);
		p_inf.INFST_VAL_CONT	 := p_inf.INFST_BASE_ICMS;-- NVL(p_inf.INFST_VAL_CONT,0) + ABS(NVL(p_inf.INFST_OUTRAS_ICMS,0));
		p_inf.INFST_VAL_SERV     := p_inf.INFST_BASE_ICMS;-- NVL(p_inf.INFST_VAL_SERV,0) + ABS(NVL(p_inf.INFST_OUTRAS_ICMS,0));
