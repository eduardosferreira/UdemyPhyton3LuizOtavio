...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_7_AJUS_I_0_TOT_PREEN'
...
PROMPT
PROMPT MAP_2_REGRA_7_AJUS_I_0_TOT_PREEN
...
DECLARE
   ...
   CURSOR c_inf
    ...
    WHERE ${FILTRO}
	  ...
	  and nvl(inf.INFST_VAL_CONT,0)    = nvl(inf.INFST_ISENTA_ICMS,0)
...
BEGIN
   ...
   LOOP
	  ...
	  v_inf.INFST_ISENTA_ICMS := nvl(v_inf.infst_val_serv,0);		 

      UPDATE openrisow.item_nftl_serv partition (${PARTICAO_INF}) inf
        SET inf.VAR05                = SUBSTR('r2015_7:' || inf.INFST_ISENTA_ICMS || '|' || inf.infst_val_desc || '|' || inf.infst_val_serv|| '|' || inf.infst_val_cont || '>>'|| inf.VAR05,1,150)
       	  , inf.infst_val_cont       = nvl(v_inf.infst_val_cont,0)
		  , inf.INFST_ISENTA_ICMS    = nvl(v_inf.INFST_ISENTA_ICMS,0)
