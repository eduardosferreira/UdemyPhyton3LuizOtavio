-- CREATE OR REPLACE 
PROCEDURE prcts_regra_44(p_inf         IN OUT NOCOPY c_inf%rowtype, p_sanea  IN  c_sanea%rowtype)
AS
...
AND p_inf.infst_dtemiss >= TO_DATE('01/01/2015','DD/MM/YYYY') AND p_inf.infst_dtemiss <= TO_DATE('31/12/2016','DD/MM/YYYY')	
  THEN
	  ...
	  IF v_fl_exists > 0 THEN
		  p_inf.update_reg        				:= 1;
		  p_inf.var05             				:= substr('prcts_regra_44u:' || p_inf.CFOP || '|' || p_inf.SERV_COD  || '>>' ||p_inf.var05,1,150);
