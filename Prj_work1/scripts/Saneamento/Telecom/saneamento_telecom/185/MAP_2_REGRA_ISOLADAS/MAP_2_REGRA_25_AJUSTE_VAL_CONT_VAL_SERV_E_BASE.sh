...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_25_AJUSTE_VAL_CONT_VAL_SERV_E_BASE'
...
PROMPT
PROMPT MAP_2_REGRA_25_AJUSTE_VAL_CONT_VAL_SERV_E_BASE
...
DECLARE
   ...
   CURSOR c_inf
    ...
    WHERE ${FILTRO}
	      AND UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) = 'UT'
	...
	AND (UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) NOT IN ('ASS') OR nf.mnfst_dtemiss >= TO_DATE('01/04/2017','DD/MM/YYYY'))
		  ...
		  AND nvl(inf.INFST_VAL_SERV,0)     =   nvl(inf.INFST_BASE_ICMS,0)
		  AND nvl(inf.INFST_OUTRAS_ICMS,0) <> 0
		  AND nvl(inf.INFST_VAL_DESC,0)     = 0 		  
		  AND nvl(inf.INFST_ISENTA_ICMS,0)  = 0
		  AND ((NVL(inf.INFST_VAL_SERV,0)  -  NVL(inf.INFST_VAL_DESC,0) <>  NVL(inf.INFST_BASE_ICMS,0)  + NVL(inf.INFST_OUTRAS_ICMS,0) + NVL(inf.INFST_ISENTA_ICMS,0)) 
		  OR   (NVL(inf.INFST_VAL_CONT,0) <>  NVL(inf.INFST_BASE_ICMS,0) +  NVL(inf.INFST_OUTRAS_ICMS,0) + NVL(inf.INFST_ISENTA_ICMS,0)));
...
BEGIN
   ...
   LOOP
	  ...
	  v_inf.INFST_VAL_CONT    := v_inf.INFST_VAL_SERV; 

	  UPDATE openrisow.item_nftl_serv inf
	  SET inf.var05             = substr('r2015_25u' ||':' || inf.INFST_OUTRAS_ICMS || '|' || inf.INFST_VAL_CONT ||'>>'||inf.var05,1,150) 			
