...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_4_AJUSTE_ITEM_0_TOT_PREEN'
...
PROMPT
PROMPT MAP_2_REGRA_4_AJUSTE_ITEM_0_TOT_PREEN
...
DECLARE
   ...
   CURSOR c_inf
    ...
    WHERE ${FILTRO}
	  ...
	  AND nvl(inf.infst_val_cont,0)     = nvl(inf.infst_outras_icms,0)   
...
BEGIN
   ...
   LOOP
	  ...
	  v_inf.infst_outras_icms := nvl(v_inf.infst_val_serv,0);		 

      UPDATE openrisow.item_nftl_serv partition (${PARTICAO_INF}) inf
        SET inf.VAR05                = SUBSTR('r2015_4:' || inf.infst_outras_icms || '|' || inf.infst_val_desc || '|' || inf.infst_val_serv|| '|' || inf.infst_val_cont || '>>'|| inf.VAR05,1,150)
