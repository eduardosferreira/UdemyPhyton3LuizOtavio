...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_CFOP_PARA_0000'
...
PROMPT
PROMPT MAP_2_REGRA_CFOP_PARA_0000
...
DECLARE
  ...
  CURSOR c_inf IS
    SELECT /*+ parallel(8,8) */
     ...
     WHERE ${FILTRO}
	   AND UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) IN ('U T', '1') 
	   ...
	   AND EXISTS (SELECT 1 FROM GFCADASTRO.TMP_ST_03 tmp WHERE UPPER(TRIM(tmp.servtl_cod)) = UPPER(TRIM(inf.serv_cod)))  
	   ...
	   AND NVL(inf.infst_val_cont, 0)      = NVL(inf.infst_val_serv, 0)  
	   AND (NVL(inf.cfop,'_0_')             != '0000'	 
			or inf.ESTB_COD != '90'
			or inf.INFST_TRIBICMS != 'P');
  ...
  BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(substr(TO_CHAR(SYSDATE, 'DD/MM/YYYY HH24:MI:SS') || ' ) ' || pDDO,
                                  1,
                                  2000));
  EXCEPTION
    WHEN OTHERS THEN
...
BEGIN
  ...
  LOOP
	...
	SET    inf.VAR05          = substr('k'||v_inf.serie||'CFOP_P_0:' || inf.cfop || '|' ||  inf.ESTB_COD || '|' ||  inf.INFST_TRIBICMS  || '>>'||inf.var05,1,150) 
		,  inf.CFOP           = '0000'
		,  inf.ESTB_COD       = '90'
		,  inf.INFST_TRIBICMS = 'P'
...
EXCEPTION
  WHEN OTHERS THEN
    ROLLBACK;
    prc_tempo('Erro : ' || SUBSTR(SQLERRM, 1, 500) || ' - rowid_inf >> ' ||
              v_inf.rowid_inf);
