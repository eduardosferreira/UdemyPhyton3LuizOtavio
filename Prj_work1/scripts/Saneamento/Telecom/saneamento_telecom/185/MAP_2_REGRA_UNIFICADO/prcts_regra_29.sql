...
BEGIN
  IF ( 
       p_sanea.serie IN ('UT')
  AND p_nf.mnfst_dtemiss >= TO_DATE('01/01/2015','DD/MM/YYYY') AND p_nf.mnfst_dtemiss <= TO_DATE('31/12/2016','DD/MM/YYYY')	
     ) 
  THEN
    ...
    THEN
		...
		SELECT
			...
			inf.var03                 , 
			inf.var04                 , 
			SUBSTR('prcts_regra_29i:' || inf.CFOP || '|' || inf.infst_val_cont || '|' || inf.INFST_VAL_SERV || '|' || inf.INFST_BASE_ICMS || '|' || inf.INFST_VAL_ICMS || '|' || inf.ESTB_COD || '|' || inf.INFST_ALIQ_ICMS || '|' || inf.INFST_TRIBICMS || '>>'|| inf.var05,1,150) AS var05   , 
		...
		p_inf.update_reg        			  := 1;
	        p_inf.VAR05                           := SUBSTR('prcts_regra_29u:' || p_inf.INFST_OUTRAS_ICMS || '|' || p_inf.infst_val_cont || '|' || p_inf.INFST_VAL_SERV || '>>'|| p_inf.VAR05,1,150);
                p_inf.INFST_OUTRAS_ICMS    			  := 0;
                p_inf.INFST_VAL_SERV      			  := NVL(p_inf.INFST_BASE_ICMS,0) + NVL(p_inf.INFST_ISENTA_ICMS,0);
                p_inf.INFST_VAL_CONT      			  := NVL(p_inf.INFST_BASE_ICMS,0) + NVL(p_inf.INFST_ISENTA_ICMS,0);		
