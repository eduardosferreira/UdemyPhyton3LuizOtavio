...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_21_DESCONTO_BASE_OUTROS'
...
PROMPT
PROMPT MAP_2_REGRA_21_DESCONTO_BASE_OUTROS
...
BEGIN
   ...
   LOOP
	...
	IF       NVL(v_item_nftl_serv.INFST_OUTRAS_ICMS,0)                               <> 0
	   ...
	   AND  (NVL(v_item_nftl_serv.INFST_BASE_ICMS,0)    <> 0 OR NVL(v_item_nftl_serv.INFST_VAL_ICMS,0) <> 0)
	   AND   NVL(v_item_nftl_serv.INFST_VAL_CONT,0)                                  = 0
	   AND   NVL(v_item_nftl_serv.INFST_VAL_SERV,0)                                  = 0
       AND   NVL(v_item_nftl_serv.INFST_ISENTA_ICMS,0)                               = 0
	THEN

		v_item_nftl_serv.update_reg        := 1;
		v_item_nftl_serv.VAR05             := SUBSTR('r2015_21i:' || v_item_nftl_serv.CFOP || '|' || v_item_nftl_serv.INFST_VAL_DESC || '|' || v_item_nftl_serv.INFST_OUTRAS_ICMS || '|' || v_item_nftl_serv.infst_val_cont || '|' || v_item_nftl_serv.INFST_VAL_SERV || '|' || v_item_nftl_serv.INFST_BASE_ICMS || '|' || v_item_nftl_serv.INFST_VAL_ICMS || '|' || v_item_nftl_serv.ESTB_COD || '|' || v_item_nftl_serv.INFST_ALIQ_ICMS || '|' || v_item_nftl_serv.INFST_TRIBICMS || '>>'|| v_item_nftl_serv.VAR05,1,150);
        
        v_item_nftl_serv.INFST_OUTRAS_ICMS := (NVL(v_item_nftl_serv.INFST_VAL_DESC,0)* -1) + NVL(v_item_nftl_serv.INFST_OUTRAS_ICMS,0);                                                 
        v_item_nftl_serv.INFST_VAL_SERV    := v_item_nftl_serv.INFST_OUTRAS_ICMS;                                                 
        v_item_nftl_serv.INFST_VAL_CONT    := v_item_nftl_serv.INFST_OUTRAS_ICMS; 
	...
	IF v_item_nftl_serv.update_reg     = 1 THEN
        ...
        v_inf.update_reg           			  := 1;
	    v_inf.VAR05                           := SUBSTR('r2015_21u:' || v_inf.INFST_OUTRAS_ICMS || '|' || v_inf.INFST_VAL_DESC || '|' || v_inf.infst_val_cont || '|' || v_inf.INFST_VAL_SERV || '>>'|| v_inf.VAR05,1,150);
        v_inf.INFST_OUTRAS_ICMS               := 0;
