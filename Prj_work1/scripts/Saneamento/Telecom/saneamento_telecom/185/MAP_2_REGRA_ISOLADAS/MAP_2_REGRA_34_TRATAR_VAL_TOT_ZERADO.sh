...
var v_msg_erro            VARCHAR2(4000) = 'MAP_2_REGRA_34_TRATAR_VAL_TOT_ZERADO'
...
PROMPT
PROMPT MAP_2_REGRA_34_TRATAR_VAL_TOT_ZERADO
...
DECLARE   
	...
	WHERE   ${FILTRO}
		...
		AND (UPPER(TRANSLATE(nf.mnfst_serie,'x ','x')) NOT IN ('ASS') OR nf.mnfst_dtemiss >= TO_DATE('01/04/2017','DD/MM/YYYY')) 
		  ...
		  AND (inf.infst_tribicms <> 'P' OR inf.estb_cod <> '90'or inf.cfop <> '0000');
    v_inf                 c_inf%ROWTYPE;
...
BEGIN
   ...
   LOOP
	...
	SET inf.var05              = substr('r2015_34u:' || inf.infst_tribicms || '|' || inf.estb_cod || '|' || inf.infst_aliq_icms || '|' || inf.cfop  || '>>' ||inf.VAR05,1,150) 	 			
